        ...
        if "PatientBirthDate" in attributes:
            ...
            if not ninety_plus:
                ...
                new_age = str(
                    int(
                        (
                            datetime.strptime(dataset.StudyDate, "%Y%m%d")
                            - datetime.strptime(fuzzed_birthdate, "%Y%m%d")
                        ).days
                        / 365
                    )
                )
...
def anonymize_all_dicoms_within_root_folder(
    ...
    for patient_index, patient in enumerate(progress(patients_folders)):
        ...
        if os.path.isfile(os.path.join(datapath, patient, "new_id.txt")):
            new_id = (
                open(os.path.join(datapath, patient, "new_id.txt"))
                .read()
                .splitlines()[0]
            )
        ...
        if not all_filenames:
            ...
            if not os.path.isdir(
                os.path.join(output_folder, patient)
            ):  # create subject dir if needed
            ...
            for study_dir in study_dirs:
                if not os.path.isdir(
                    os.path.join(output_folder, patient, study_dir)
                ):  # create study dir if needed
                ...
                if fuzz_acq_dates:
                    ...
                    # shift it
                    fuzzed_study_date = shift_date_by_some_days(
                        original_study_date, int(all_date_offsets[patient_index])
                    )
                    # rename dir
                    fuzzed_study_dir = (
                ...
                for series_dir in series_dirs:
                    ...
                    for filename in all_filenames_series:
                        new_SOPInstanceUID = pydicom.uid.generate_uid(
                            pydicom.uid.PYDICOM_ROOT_UID
                        )
        ...
        if rename_patient_directories:
            try:
                os.replace(
                    os.path.join(output_folder, patient),
                    os.path.join(output_folder, "sub-" + new_id),
                )
            except OSError:  # if destination dir already exists
                os.replace(
                    os.path.join(output_folder, patient),
                    os.path.join(output_folder, "sub-" + new_id + "_2"),
                )
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        "Anonymize DICOM files for subsequent upload to Kheop."
    )
...
def main():
    """Main function of the script that calls :func:`anonymize_all_dicoms_within_root_folder`."""
    # Create the parser object and parse the arguments
    parser = get_parser()
    ...
    print(
        "Anonymizing dicom files within path {} into {}".format(
            os.path.abspath(data_path), os.path.abspath(output_folder)
        )
    )
